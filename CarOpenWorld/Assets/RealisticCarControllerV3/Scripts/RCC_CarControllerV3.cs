...
public class RCC_CarControllerV3 : RCC_Core {
	...
	public bool useCounterSteering = true;								// Applies counter steering when vehicle is
                                                                        // ing. It helps to keep the control fine of the vehicle.
...
//		allWheelColliders [i].ApplyMotorTorque ((direction * allWheelColliders[i].powerMultiplier * (1f - clutchInput) * throttleInput * (1f + boostInput) * (engineTorqueCurvee.Evaluate(engineRPM) * gears[currentGear].torqueCurve.Evaluate((RearLeftWheelCollider.wheelRPMToSpeed + RearRightWheelCollider.wheelRPMToSpeed) / 2f) * finalRatio)) / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity));
	...
	public void ResetRobot()                //Abdul
	{
		//if (speed < 50.75f && !rigid.isKinematic)
		//{
			Debug.Log("hain");
			//resetTime += Time.deltaTime;
			//if (resetTime > 0.1f)
			...
			resetTime = 0f;
			///GameAnalytics
			//PlayerPrefs.SetInt("ResetBot", PlayerPrefs.GetInt("ResetBot") + 1);
